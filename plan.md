...
| **STT Engine** | Speech-to-Text | **`whisper.cpp` compiled to WASM** (C++/`ggml`, WASM SIMD), run synchronously in a dedicated Worker. Single-threaded for now — see §4.7. |
...
7. **Shipped single-threaded for now, not by choice.** The build was compiled twice: once with `USE_PTHREADS=1` (matching the upstream whisper.cpp WASM demo) and once with pthreads removed entirely. The pthreads build hung indefinitely at the inference step — reproduced identically across two Emscripten versions (6.0.6 and 3.1.74) and in both plain Node and headless Chromium with `crossOriginIsolated: true` confirmed, which points at the sandboxed dev/build environment restricting native thread/worker creation rather than a code bug. The pthreads-free build worked immediately and correctly transcribed real speech end-to-end. **Follow-up:** re-enable `USE_PTHREADS=1` and verify in a normal (non-sandboxed) browser — real Chrome very likely handles this fine, since it's the exact pattern whisper.cpp's own official demo uses in production. Until verified, single-threaded is what's actually confirmed working.
...
9. **Fixed-size 5-second transcription windows cut mid-sentence.** Whisper gets no context across window boundaries, so text that spans a chunk boundary comes out truncated/garbled at the edges (observed in testing: "We are checking that with --" / "We are checking that we're" — same audio, two different truncated results from mic vs. participants chunking slightly out of phase). Acceptable for the Phase 2 milestone; a real fix is VAD-based segmentation or overlapping windows, not naive fixed-size chunking — flag as a Phase 2 follow-up, not Phase 3+.
...
- Fixed 5s rolling windows per channel (`src/hooks/useTranscription.ts`), chunked/near-real-time rather than true streaming — see §4.9 for the chunk-boundary quality tradeoff.
