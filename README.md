...
- **"Finalizing transcript" indicator** — local transcription can still have a backlog draining after you hit Stop (whisper.cpp processes chunks through one serial queue; if a call runs behind real-time, Stop doesn't clear that backlog, it just adds to the end of it — this is what was behind a real user seeing roughly a minute's delay before the final transcript settled). Generate Summary and Ask are now disabled with a visible "Finalizing transcript…" banner until that backlog actually drains, so neither runs against a partial transcript.
